...
const Shop = ({ addtocart, searchResults }) => {
  ...
  const [currentCategory, setCurrentCategory] = useState(''); // Etat pour la catégorie actuelle
  ...
  const handleLike = async (productId) => {
    ...
    try {
      ...
      if (response.ok) {
        fetchProducts(currentCategory); // Recharger les produits après avoir aimé
