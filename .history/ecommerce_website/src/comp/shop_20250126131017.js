...
const Shop = ({ addtocart, searchResults }) => {
  ...
  const [currentCategory, setCurrentCategory] = useState(''); // Ajout d'un état pour la catégorie actuelle
  ...
  useEffect(() => {
    fetchProducts(currentCategory); // Mise à jour de l'effet pour recharger les produits en fonction de la catégorie
  }, [currentCategory]); // Ajouter currentCategory comme dépendance
  ...
  const filtercate = (category) => {
    setCurrentCategory(category); // Mise à jour de la catégorie actuelle
  };

  const allcatfilter = () => {
    setCurrentCategory(''); // Réinitialiser la catégorie actuelle
  };
  ...
  const handleLike = async (productId) => {
    ...
    try {
      ...
      if (response.ok) {
        fetchProducts(currentCategory); // Recharger les produits après avoir aimé un produit
