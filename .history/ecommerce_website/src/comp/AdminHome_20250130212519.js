...
import jwt_decode from 'jwt-decode'; // Import correct
...
const AdminHome = () => {
  ...
  useEffect(() => {
    const fetchAdminInfo = () => {
      ...
      try {
        const decoded = jwt_decode(token); // Utilisation de jwt_decode
        setAdmin({ name: decoded.name, role: decoded.role });
      } catch (error) {
  ...
  return (
    <div className="admin-home">
      <h1>Bienvenue, {admin?.name || 'Admin'} ({admin?.role || 'Role inconnu'})</h1>
