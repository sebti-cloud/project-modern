...
import Banner from './Banner'; // Ajout de l'importation du composant Banner
...
const Shop = ({ addtocart, searchResults }) => {
  ...
  const [banners, setBanners] = useState([]); // Ajout de l'état pour les bannières
  ...
  const fetchBanners = async () => {
    try {
      ...
      const homeBanners = response.data.filter(banner => banner.page === 'home');
...
const Shop = ({ addtocart, searchResults }) => {
  ...
  useEffect(() => {
    ...
    fetchCategories();
    getUserId();
    fetchBanners(); // Appel pour récupérer les bannières
  }, []);
      ...
      <div className='shop'>
        ...
        <div className='container'>
          <div className='left_box'>
            ...
            <div className='banners'>
              {banners.map((banner, index) => (
                <Banner key={index} image={banner.image_url} link={banner.link} />
              ))}
            </div>
          </div>
          <div className='right_box'>
