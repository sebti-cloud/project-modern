...
const Shop = ({ addtocart, searchResults, setCartCount }) => {
  ...
  const [cartCount, setCartCountState] = useState(parseInt(localStorage.getItem('cartCount')) || 0); // Initialiser à partir de localStorage
  ...
  useEffect(() => {
    ...
    updateCartCount(); // Mettre à jour le compteur de panier au chargement
  }, []);
  ...
  const addToCart = (product) => {
    ...
    setCartCountState(cartCount + 1); // Incrémenter le compteur de panier
    localStorage.setItem('cartCount', cartCount + 1); // Sauvegarder dans localStorage
    setCartCount(cartCount + 1); // Synchroniser avec l'état global du panier
  };

  const updateCartCount = () => {
    const storedCart = JSON.parse(localStorage.getItem('cart')) || [];
    const count = storedCart.reduce((acc, product) => acc + product.qty, 0);
    setCartCountState(count);
    setCartCount(count); // Synchroniser avec l'état global du panier
  };
