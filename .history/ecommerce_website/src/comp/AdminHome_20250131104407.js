...
const AdminHome = () => {
  ...
  const renderGreeting = () => {
    ...
    const { username, sexe, role } = admin;
    const name = username || 'Admin';
    const roleText = role || 'Rôle inconnu';

    if (sexe === 'Homme') {
      return `Bonjour, Mr ${name} (${roleText})`;
    } else if (sexe === 'Femme') {
      return `Bienvenue, Mme ${name} (${roleText})`;
    } else {
      return `Bienvenue, ${name} (${roleText})`;
    }
  };
