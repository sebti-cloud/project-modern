...
const Cart = ({ cart, setCart }) => {
  ...
  const updateCartCount = () => {
    const count = cart.reduce((price, item) => price + item.qty, 0);
    setCartCount(count);
  };
  ...
  const handleCheckout = async (e) => {
    ...
    if (response.ok) {
      ...
      if (userInfo.paymentMethod === 'online payment') {
        ...
        setCheckout(false);
        setModalIsOpen(true);
        setCartCount(0); // Réinitialiser le compteur de panier
  ...
  const incqty = async (product) => {
    ...
    if (exist) {
      ...
      updateCartCount(); // Mettre à jour le compteur de panier après l'incrémentation
    }
  };
  const decqty = async (product) => {
    ...
    if (exist && exist.qty > 1) {
      ...
      setCart(cart.map((curElm) => curElm.id === product.id ? { ...exist, qty: updatedQty } : curElm));
      updateCartCount(); // Mettre à jour le compteur de panier après la décrémentation
    } else {
  ...
  const removeproduct = async (product) => {
    ...
    updateCartCount(); // Mettre à jour le compteur de panier après la suppression
  };
