        ...
        <table>
          ...
          <tbody>
            {purchases.map((purchase, index) => {
              const productDetails = getProductDetails(purchase.product);
              return (
                <tr key={index}>
                  <td>
                    {productDetails && productDetails.images && productDetails.images.length > 0 ? (
                      <img src={`http://localhost:3001${productDetails.images[0]}`} alt={productDetails.name} width="50" />
                    ) : (
                      'No Image'
                    )}
                  </td>
                  <td>{productDetails ? productDetails.name : purchase.product}</td>
                  <td>{productDetails ? `${productDetails.price} MAD` : 'N/A'}</td>
                  <td>{productDetails ? productDetails.category : 'N/A'}</td>
            ...
            {likedProducts.map((product, index) => {
              ...
              return (
                <tr key={index}>
                  ...
                  <td>{productDetails ? productDetails.name : product.name}</td>
                  <td>{productDetails ? `${productDetails.price} MAD` : 'N/A'}</td>
                  <td>{productDetails ? productDetails.category : 'N/A'}</td>
                </tr>
              );
