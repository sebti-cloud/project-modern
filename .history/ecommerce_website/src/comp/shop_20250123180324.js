...
const Shop = ({ addtocart, searchResults }) => {
  ...
  const fetchProducts = async (category = '') => {
    try {
      ...
      // Convertir le champ `images` en tableau
      const products = data.map(product => ({
        ...product,
        images: product.images ? JSON.parse(product.images) : []
      }));
      setShop(Array.isArray(products) ? products : []);
    } catch (error) {
  ...
  // Définition de settings pour react-slick
  const settings = {
    dots: true,
    infinite: true,
    speed: 500,
  ...
  // Fonction pour afficher les images des produits
