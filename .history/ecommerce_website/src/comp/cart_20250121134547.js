...
const Cart = ({ cart, setCart }) => {
  ...
  const [transactionNumber, setTransactionNumber] = useState(''); // État pour le numéro de transaction
  ...
  const handleCheckout = async (e) => {
    ...
    if (response.ok) {
      ...
      setTransactionNumber(data.transactionNumber); // Stocker le numéro de transaction
  ...
  const copyToClipboard = (text, type) => {
    console.log(`Copying ${type}:`, text); // Ajouter un log pour vérifier la valeur copiée
    ...
    <>
      ...
      <Modal isOpen={modalIsOpen} onRequestClose={() => setModalIsOpen(false)} contentLabel="Modal de Suivi de Commande" className="modal" overlayClassName="overlay">
        ...
        <h3>{trackingNumber}</h3>
          <button onClick={() => copyToClipboard(trackingNumber, 'Numéro de suivi')} className="copy-button">
            <FaCopy /> Copier
          </button>
        </div>
