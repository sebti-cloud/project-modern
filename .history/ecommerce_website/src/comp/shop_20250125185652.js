...
<ul>                        {curElm.images.map((_, dotIndex) => (
                          <li key={dotIndex} className={dotIndex === autoplayIndex ? 'slick-active' : ''}>
                            <button type="button" onClick={() => setAutoplayIndex(dotIndex)}>•</button>
                          </li>
                        ))}
                      </ul>
                      <h3>{curElm.name}</h3>
