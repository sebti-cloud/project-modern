...
const Home = ({ addtocart, searchResults }) => {
    ...
    const fetchProducts = async (category = '') => {
        try {
            ...
            console.log(data); // Affiche les données des produits dans la console pour vérifier les chemins d'images
    ...
    const renderProductImage = (imagePath) => {
        console.log('Chemin de l\'image:', imagePath); // Affiche le chemin de l'image dans la console
        const fullImagePath = `http://localhost:3001${imagePath.trim()}`;
        console.log('Chemin complet de l\'image:', fullImagePath); // Vérifiez si le chemin complet est valide
                        ...
                        <div className="box">
                            ...
                            {Array.isArray(newProduct) && newProduct.map((curElm) => (
                                <div key={curElm.id} className="productbox">
                                    <div className="img_box">
                                        {/* Correction ici : utilisez curElm.image au lieu de product.image */}
                                        <img src={renderProductImage(curElm.image)} alt={curElm.name} />
                                    </div>
