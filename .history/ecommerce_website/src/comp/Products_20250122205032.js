...
const Products = () => {
    ...
    const [selectedFiles, setSelectedFiles] = useState([]); // Modifier pour permettre plusieurs fichiers
    ...
    const handleFileChange = (e) => {
        setSelectedFiles(Array.from(e.target.files)); // Modifier pour permettre plusieurs fichiers
    };
    ...
    const handleAddProduct = async (e) => {
        ...
        selectedFiles.forEach(file => {
            formData.append('images', file); // Utiliser 'images' pour permettre plusieurs fichiers
        });
        try {
            const response = await fetch('http://localhost:3001/api/products', {
                method: 'POST',
        ...
        <div className="admin-dashboard">
            ...
            <div className="products">
                ...
                <form onSubmit={editProduct ? handleUpdateProduct : handleAddProduct}>
                    ...
                    <select name="types" value={editProduct ? editProduct.types : newProduct.types} onChange={handleChange} required multiple>
                    ...
                    <input type="file" name="images" onChange={handleFileChange} multiple required={!editProduct} />
                            ...
                            <tr key={product.id}>
                                <td>
                                    {product.images && product.images.map((image, index) => (
                                        <img key={index} src={`http://localhost:3001${image}`} alt={product.name} className="product-image" />
                                    ))}
                                </td>
                                <td>{product.name}</td>
