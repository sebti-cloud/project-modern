...
const ProductForm = ({ product, saveProduct }) => {
  ...
  const [lowStockThreshold, setLowStockThreshold] = useState(product ? product.lowStockThreshold : 0); // Ajouter le champ seuil de stock bas
  const [category, setCategory] = useState(product ? product.category : '');
  const [details, setDetails] = useState(product ? product.details : '');
  const [supplierId, setSupplierId] = useState(product ? product.supplierId : ''); // Ajouter le champ fournisseur
  ...
  const handleSubmit = (e) => {
    ...
    formData.append('lowStockThreshold', lowStockThreshold); // Ajouter le champ seuil de stock bas
    formData.append('category', category);
    formData.append('details', details);
    formData.append('supplierId', supplierId); // Ajouter le champ fournisseur
