...
app.put('/api/products/:id/like', authenticateToken, async (req, res) => {
  const { id } = req.params;
  const userId = req.user.id; // Assurez-vous que l'ID de l'utilisateur est récupéré à partir du jeton
  ...
  try {
    ...
    // Enregistrement du produit aimé pour l'utilisateur connecté
...
// Route pour obtenir les produits aimés par un utilisateur connecté
