...
app.post('/api/products', upload.array('images', 10), async (req, res) => {
  const { name, category, types, price, details, quantity, lowStockThreshold } = req.body; // Inclure le champ lowStockThreshold

  // Traiter les chemins des images
  const images = req.files
    ? req.files.map(file => `/uploads/${file.filename}`)
    : []; // Par défaut, un tableau vide si aucune image n'est uploadée

  try {
    // Insérer les données dans la base de données
    const query = `
      INSERT INTO products (name, category, types, price, details, images, quantity, low_stock_threshold, created_at) 
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW()) RETURNING id
    `;
    const values = [
      name,
      category,
      types ? JSON.parse(types).map(type => type.trim()) : null, // Convertir JSON en tableau
      price,
      details,
      `{${images.map(img => `"${img}"`).join(',')}}`, // Transformer les chemins en tableau PostgreSQL
      quantity, // Inclure la quantité
      lowStockThreshold // Inclure le seuil de stock bas
    ];
  ...
  try {
    ...
    try {
      ...
      const updateProductQuery = `
        UPDATE products SET name = $1, category = $2, types = $3, price = $4, details = $5, image = COALESCE($6, image)
        WHERE id = $7
        RETURNING *
      `;
      const result = await client.query(updateProductQuery, [name, category, types ? JSON.parse(types) : [], price, details, image, id]);
      ...
      res.status(200).send({ message: 'Product updated successfully', product: result.rows[0] });
    } catch (err) {
      await client.query('ROLLBACK');
...
app.put('/api/products/:id', upload.array('images', 10), async (req, res) => {
  ...
  const images = req.files
    ? req.files.map(file => `/uploads/${file.filename}`)
    : null; // Par défaut, un tableau vide si aucune image n'est uploadée
  ...
  try {
    ...
    try {
      ...
      // Obtenir l'ancienne quantité
      const oldQuantityResult = await client.query('SELECT quantity FROM products WHERE id = $1', [id]);
      const oldQuantity = oldQuantityResult.rows[0].quantity;

      const updateProductQuery = `
        UPDATE products 
        SET name = $1, category = $2, types = $3, price = $4, details = $5, images = COALESCE($6, images), quantity = $7, low_stock_threshold = $8, updated_at = NOW() 
        WHERE id = $9
        RETURNING *
      `;
      const result = await client.query(updateProductQuery, [
        ...
        price, 
        details, 
        images ? `{${images.map(img => `"${img}"`).join(',')}}` : null, 
        quantity, 
        lowStockThreshold, 
        id
      ]);
      ...
      const stockMovementQuery = `
        ...
        VALUES ($1, $2, $3)
      `;
      await client.query(stockMovementQuery, [id, Math.abs(quantityDifference), movementType]);
...
app.get('/api/user-activities/purchases/:userId', async (req, res) => {
  const { userId } = req.params;
  const query = 'SELECT * FROM user_purchases WHERE user_id = $1';
  try {
