...
app.put('/api/products/:id', async (req, res) => {
  const { id } = req.params;
  const { name, price, quantity, lowStockThreshold, category, details, supplierId, types } = req.body;

  console.log('Received PUT request for product ID:', id); // Log l'ID du produit
  console.log('Received data:', req.body); // Log les données reçues

  try {
    const query = `
      UPDATE products
      SET name = $1, price = $2, quantity = $3, low_stock_threshold = $4, category = $5, details = $6, supplier_id = $7, types = $8, updated_at = NOW()
      WHERE id = $9
      RETURNING *
    `;
    const values = [name, price, quantity, lowStockThreshold, category, details, supplierId, types, id];
    const result = await pool.query(query, values);

    if (result.rowCount === 0) {
      console.log('Product not found with ID:', id); // Log si le produit n'est pas trouvé
      return res.status(404).json({ message: 'Product not found' });
    }

    console.log('Product updated successfully:', result.rows[0]); // Log le produit mis à jour
