...
const multer = require('multer');
const upload = multer({ dest: 'uploads/' }); // Configurer le chemin pour les fichiers uploadés
const pool = require('./db'); // Assurez-vous que la connexion à la base de données est correcte
...
app.post('/api/products', upload.array('images', 10), async (req, res) => {
  ...
  try {
    ...
    const query = `
      INSERT INTO products (name, category, types, price, details, images, created_at) 
      VALUES ($1, $2, $3, $4, $5, $6, NOW())
    `;
    const values = [
      name,
      category,
      types ? JSON.stringify(JSON.parse(types)) : '[]', // Convertir `types` en JSON
      price,
      details,
