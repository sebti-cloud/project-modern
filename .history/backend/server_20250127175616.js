...
app.post('/api/products', upload.array('images', 10), async (req, res) => {
  const { name, category, types, price, details, quantity, lowStockThreshold, supplierId } = req.body; // Inclure le champ supplierId
  ...
  try {
    ...
    const values = [
      name,
      category,
      types ? JSON.parse(types).map(type => type.trim()) : null,
      price,
      details,
      `{${images.map(img => `"${img}"`).join(',')}}`,
      quantity,
      lowStockThreshold,
      supplierId // Inclure le supplier_id
    ];
