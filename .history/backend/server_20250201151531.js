...
const uploadSlider = multer({ storage: storageSliders });


// Routes pour les bannières
// Route pour ajouter une bannière
app.post('/api/banners', upload.single('image'), async (req, res) => {
  const { title, link, position, page } = req.body;
  if (!req.file) {
    return res.status(400).json({ message: 'No file uploaded' });
  }
  const image_url = `/uploads/banners/${req.file.filename}`;
  try {
    const query = 'INSERT INTO banners (title, image_url, link, position, page) VALUES ($1, $2, $3, $4, $5)';
    await pool.query(query, [title, image_url, link, position, page]);
...
// Route pour récupérer les bannières
app.get('/api/banners', async (req, res) => {
  console.log('Request received to fetch banners');
  try {
    const result = await pool.query('SELECT * FROM banners ORDER BY position');
    console.log('Banners fetched successfully:', result.rows);
...
app.put('/api/banners/:id', uploadBanner.single('image'), async (req, res) => {
  ...
  try {
    const query = `
      UPDATE banners
      SET title = $1, link = $2, position = $3, image_url = COALESCE($4, image_url), updated_at = CURRENT_TIMESTAMP
      WHERE id = $5
    `;
    await pool.query(query, [title, link, position, image_url, id]);
...
app.post('/api/sliders', uploadSlider.single('image'), async (req, res) => {
  const { title, link, position } = req.body;
  const image_url = `/uploads/sliders/${req.file.filename}`;
  try {
    const query = 'INSERT INTO sliders (title, image_url, link, position) VALUES ($1, $2, $3, $4)';
    await pool.query(query, [title, image_url, link, position]);
...
app.get('/api/sliders', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM sliders ORDER BY position');
    res.status(200).json(result.rows);
  } catch (error) {
...
app.put('/api/sliders/:id', uploadSlider.single('image'), async (req, res) => {
  const { id } = req.params;
  const { title, link, position } = req.body;
  const image_url = req.file ? `/uploads/sliders/${req.file.filename}` : undefined;
  try {
    const query = `
      UPDATE sliders
      SET title = $1, link = $2, position = $3, image_url = COALESCE($4, image_url), updated_at = CURRENT_TIMESTAMP
...
app.post('/api/admins', async (req, res) => {
  const { firstName, lastName, email, username, phone, role, password, sexe } = req.body; // Inclure le sexe
  
  try {
    const hashedPassword = await bcrypt.hash(password, 10); // Hacher le mot de passe fourni
    const query = 'INSERT INTO admins (firstName, lastName, email, username, phone, password, role, sexe) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)';
    const values = [firstName, lastName, email, username, phone, hashedPassword, role, sexe];
    await pool.query(query, values);
