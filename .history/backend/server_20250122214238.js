...
app.post('/api/products', upload.array('images', 5), async (req, res) => {
  ...
  try {
    const query = 'INSERT INTO products (name, category, types, price, details, images, created_at) VALUES ($1, $2, $3, $4, $5, $6, NOW())';
    const values = [name, category, types ? JSON.parse(types) : [], price, details, images];
  ...
  try {
    ...
    try {
      ...
      const updateProductQuery = `
        UPDATE products SET name = $1, category = $2, types = $3, price = $4, details = $5, images = COALESCE($6, images)
        WHERE id = $7
        RETURNING *
      `;
      const result = await client.query(updateProductQuery, [name, category, types ? JSON.parse(types) : [], price, details, images, id]);
...
app.put('/api/products/:id/like', authenticateToken, async (req, res) => {
  const { id } = req.params;
  const userId = req.user.id; // Récupérer l'ID utilisateur depuis le jeton
  ...
  try {
    ...
    // Ajouter le produit aimé dans `user_liked_products`
    const likeQuery = 'INSERT INTO user_liked_products (user_id, product) VALUES ($1, $2)';
    const likeValues = [userId, id];
...
app.put('/api/cart/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE cart SET qty = $1 WHERE product_id = $2 RETURNING *',
      [qty, id]
    );
...
app.get('/api/user/liked-products', authenticateToken, async (req, res) => {
  ...
  try {
    const query = `
      SELECT products.*
      FROM products
      JOIN user_liked_products ON products.id = user_liked_products.product_id
      WHERE user_liked_products.user_id = $1
    `;
    const values = [userId];
    const result = await pool.query(query, values);
    res.status(200).json(result.rows);
  } catch (err) {
    res.status(500).send({ message: 'Failed to fetch liked products', error: err });
  }
});
