...
app.post('/api/products', upload.array('images', 10), async (req, res) => {
  ...
  try {
    ...
    const query = `
      INSERT INTO products (name, category, types, price, details, images, quantity, lowStockThreshold, created_at) 
...
app.put('/api/products/:id', upload.array('images', 10), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const updateProductQuery = `
        UPDATE products 
        SET name = $1, category = $2, types = $3, price = $4, details = $5, images = COALESCE($6, images), quantity = $7, lowStockThreshold = $8, updated_at = NOW() 
        WHERE id = $9
        RETURNING *
      `;
      const result = await client.query(updateProductQuery, [name, category, types ? JSON.parse(types) : [], price, details, images ? `{${images.map(img => `"${img}"`).join(',')}}` : null, quantity, lowStockThreshold, id]);
