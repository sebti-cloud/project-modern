...
const cookieParser = require('cookie-parser');  // Ajout de cookie-parser pour la gestion des cookies
const authRoutes = require('./routes/authRoutes');  // Import des routes d'authentification
const adminRoutes = require('./routes/adminRoutes');  // Import des routes d'administration
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    cb(null, Date.now() + path.extname(file.originalname)); // Génère un nom de fichier unique
  },
});
...
// Appel initial pour mettre à jour les produits vieux de plus de trois mois
...
// Appeler la fonction périodiquement
updateOldProducts();
...
app.put('/api/products/:id/like', async (req, res) => {
  ...
  try {
    ...
    if (updatedProduct.likes > 5 && !updatedProduct.types.includes('top')) {
      ...
      await client.query(updateTypeQuery, updateTypeValues);
    }
